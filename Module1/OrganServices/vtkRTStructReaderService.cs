...
{
    public class vtkRTStructReaderService 
    {
        public Dictionary<string, StructureMeta> Execute(string RtStructPath)
        {
            return GetStructList(RtStructPath);
        }

        private static Dictionary<string, StructureMeta> GetStructList(string RtStructPath)
        {
            DICOMSelector selector = DICOMObject.Read(RtStructPath).GetSelector();
            ...
            {
                return structureSetMeta.Structures;
            }
            else { return null; }
